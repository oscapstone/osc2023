...
_start:
    adr x10, .          //x10=0x80000,系統預設把bootloader放到0x80000
    ldr x11, =_blsize 
    add x11, x11, x10 
...
moving_relo:
    ...
    b moving_relo
end_relo:
    ldr x14, =_bl_entry    //jump to boot part,_bl_entry原本就是相對位子,所以搬完後會跳到正確地址
    br x14
...
.globl _start_bl
    ldr x20, =_dtb      
    str x0, [x20]       //_dtb指向的記憶體位子裡存的是dtb 的loading address
    mrs    x20, mpidr_el1        
