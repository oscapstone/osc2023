...
void traverse_device_tree(void *dtb_ptr, dtb_callback callback)
{
    ...
    uint32_t struct_size = uint32_endian_big2lttle(header->size_dt_struct);//size_dt_struct:size of the structure block 
    char* dt_struct_ptr = (char*)((char*)header + uint32_endian_big2lttle(header->off_dt_struct));//off_dt_struct: offset to structure
    char* dt_strings_ptr = (char*)((char*)header + uint32_endian_big2lttle(header->off_dt_strings));//size_dt_strings : size of the strings block
    ...
    while(pointer < end)
    {
        ...
        {
            callback(token_type, pointer, 0, 0);//typedef void (*dtb_callback)(uint32_t node_type, char *name, void *value, uint32_t name_size);
