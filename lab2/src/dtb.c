...
struct fdt_header {
    ...
    uint32_t totalsize; // device tree size
    uint32_t off_dt_struct; // dt_struct offset
    uint32_t off_dt_strings; // dt_strings offset
    uint32_t off_mem_rsvmap; // reserve mamory, kernel do not map
    uint32_t version; // device tree version
    uint32_t last_comp_version; // compatible version
    uint32_t boot_cpuid_phys; // use for multi-core
    uint32_t size_dt_strings; // strings size
    uint32_t size_dt_struct; // struct size
};
...
void traverse_device_tree(void *dtb_ptr, dtb_callback callback)
{
    ...
    {
        uart_printf("traverse_device_tree : wrong magic in traverse_device_tree\n");
        return;
    }
...
void dtb_callback_initramfs(uint32_t node_type, char *name, void *value, uint32_t name_size) {
    ...
    if(node_type==FDT_PROP && strcmp(name,"linux,initrd-start")==0)
    {
        ...
        // uart_printf("CPIO address:%x\n", CPIO_DEFAULT_PLACE);
        // uart_printf("CPIO content:%s\n", CPIO_DEFAULT_PLACE);
    }
}