		...
		*tag = b2l_32(*tag);
		...
		switch(*tag){
			...
			case FDT_PROP:
				cur += 4;
				prop = (fdt_prop*) cur;
				prop->nameoff = b2l_32(prop->nameoff);
				prop->len = b2l_32(prop->len);
				uart_puts("Property: ");
				...
				uart_putsn(cur, prop->len);
				uart_puts("\n");
				cur += prop->len;
				cur += (4 - (prop->len % 4)) % 4;	// Padding to 4.
				break;
		...
		*tag = b2l_32(*tag);
		switch(*tag){
			...
			case FDT_PROP:
				...
				//uart_puti(prop->len);
				//uart_puts("\n");
				if(!strcmp(str_lo + prop->nameoff, name)){
					//uart_puts(str_lo + prop->nameoff);
					//uart_puts("TRUE start callback\n");
					fn((void*)cur, prop->len);
					return 0;
				}
				if(prop->len == 0){
					break;
