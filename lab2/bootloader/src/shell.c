...
void do_cmd_loadimg()
{
    char* bak_dtb = _dtb;  // 儲存原先的 _dtb 變數值
    char c;
    unsigned long long kernel_size = 0; // 用來儲存要載入的映像檔案的大小
    char* kernel_start = (char*) (&_start); // 用來儲存要載入的映像檔案的起始位址
    uart_puts("Please upload the image file.\r\n");
    for (int i=0; i<8; i++)  // 讀取映像檔案大小
    {
        c = uart_getc();
        kernel_size += c<<(i*8);
    }
    for (int i=0; i<kernel_size; i++)  // 讀取映像檔案資料
    {
        c = uart_getc();
        kernel_start[i] = c;
    }
    ...
    ((void (*)(char*))kernel_start)(bak_dtb); // 跳轉到新的映像檔案
}
