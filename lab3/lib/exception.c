...
void sync_64_router() {
    disable_interrupt();
    unsigned long long spsr_el1;
	asm volatile("mrs %0, SPSR_EL1\n\t" : "=r" (spsr_el1) :  : "memory");       //:->output ::->input
    ...
    //current processor’s state(PSTATE)
    uart_send_string("spsr_el1: ");     // EL1 configuration, spsr_el1[9:6]=4b0 to enable interrupt
    printhex(spsr_el1);
    uart_send_string("\n");
    
    //exception return address to EL1
    uart_send_string("elr_el1: ");
    printhex(elr_el1);
    uart_send_string("\n");
    
    //(if is synchronous ecp or SError itr)cause of that exception 
