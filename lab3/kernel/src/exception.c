...
//通過 msr 指令來操作系統寄存器來控制中斷。在 ARM64 架構中，daif 寄存器包含了四個位元用來控制不同類型的中斷。
//daifclr 操作用來清除中斷位，daifset 操作用來設置中斷位。通過將 0xf 作為參數傳遞給這些操作，可以同時清除或設置所有中斷位
//el1_interrupt_enable() 函式將所有中斷位清除以啟用中斷，而 el1_interrupt_disable() 函式將所有中斷位設置以禁用中斷。
