...
{
    list_head_t      listhead;                            // Doubly linked list for freelist node. 
    thread_context_t context;                             // Thread registers
    char*            data;                                // Process itself data
    unsigned int     datasize;                            // Process data size
    ...
    void             (*signal_handler[SIGNAL_MAX+1])();   // Signal handlers for different signal. An array of pointers, used to store the pointers of handlers for different Signal events.
    int              sigcount[SIGNAL_MAX+1];              // Signal Pending buffer. Used to record the number of times each Signal event is triggered.
    void             (*curr_signal_handler)();            // Allow Signal handler overwritten by others. Points to the currently executing Signal handler.
    int              signal_inProcess;                    // Signal Processing Lock. Used to determine whether there is currently Signal processing in progress.
