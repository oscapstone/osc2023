...
void sc_early_init(void){
    sc_frame_ents = simple_malloc(sizeof(sc_frame_ent)* frame_ents_size);
...
void *sc_alloc(int size){
    sc_header *hdr;
    uint8 size_idx = find_size_idx(size);
    ...
    hdr = list_first_entry(&sc_freelists[size_idx], sc_header, list);
    list_del(&hdr->list);
